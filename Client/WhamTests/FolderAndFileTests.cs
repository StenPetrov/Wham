...
namespace WhamTests
{
    ...
    public class FolderAndFileTests
    {
        ...
        public void TestFolderVariables ()
        {
            ...
            var fullFolder = Template.Parse ("{% Folder 'Test' %}{{ output.currentFolder }}{% endFolder %}").Render ();
            Assert.IsNotEmpty (fullFolder);
            Assert.IsNotEmpty (parentFolder);
        ...
        public void TestFileOutput ()
        {
            ...
            var fileParsed = Template.Parse ("{% Folder Test %} {% File 'outFile.txt' %}{{ output.fileName }}{% endFile %} {% endFolder %}").Render ();
        ...
        {
            var namedStream = SetupFakeOutput ();
            var fileParsed = Template.Parse ("{% Folder Test %}{% assign fn='file.txt'%}{% File fn %}{{ output.fileName }}{% endFile %} {% endFolder %}").Render ();
        ...
        {
            var namedStream = SetupFakeOutput ();
            var fileParsed = Template.Parse ("{% Folder Test %}{% assign fn = 'file' | Append: '.txt' %}{% File fn %}{{ output.fileName }}{% endFile %} {% endFolder %}").Render ();
        ...
        {
            var namedStream = SetupFakeOutput ();
            var fileParsed = Template.Parse ("{% Folder Test %} {% Trace schema -%}" +
                                             "{% assign csFile = schema | BaseClassFullName | Append: '.cs' -%} {% File csFile %}{% include 'CS_ClassTemplate.dlq'-%}{% endFile %}{% endFolder %}").Render ();

            Assert.IsNotNull (namedStream.Name);
            Assert.IsTrue (string.IsNullOrWhiteSpace (fileParsed), "Expected empty: " + fileParsed);
            Assert.IsTrue (namedStream.WrittenContents.StartsWith ("using"), "Expected to start with 'using': " + namedStream.WrittenContents);
        }
